...
- **Seq invariants:** `isValidSeq` requires `Number.isSafeInteger && >= 0`. Server ingress rejects invalid seq. `lastProcessedInput` only advances via `Math.max`
