...
1.5. Consume one move input from agent's moveQueue (per-tick input model)
...
- **Per-tick movement (Gambetta reconciliation):** Client sends `move` messages (with seq number) every 125ms while a key is held. Server consumes one from `agent.moveQueue` per tick, advancing `agent.lastProcessedInput`. Client reconciles by accepting server position as baseline, pruning acknowledged inputs, and replaying the rest. `move:stop` with seq flushes server queue and client pending buffer. No `heldDirection` — the key-state model was replaced
- **Seq invariants:** `isValidSeq` requires `Number.isSafeInteger && >= 0`. Server ingress rejects `seq <= lastProcessedInput` and `seq <= lastQueuedSeq` (monotonic). Tick Phase 1.5 drains stale inputs as defense-in-depth. `lastProcessedInput` only advances via `Math.max`. When Phase 2 plan commands discard the moveQueue, `lastProcessedInput` is advanced to the max discarded seq so the client can prune
...
- Input uses held-key tracking (`keydown`/`keyup` Set) for local prediction and sends per-tick `move` messages (with seq number) from `update()` while keys are held — not `keydown` repeat events (OS repeat has variable initial delay and rate)
